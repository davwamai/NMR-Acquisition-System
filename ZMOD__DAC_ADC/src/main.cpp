...
void writeADCData(ZMODADC1410 *padcZmod, std::string filename, uint32_t *acqBuffer, uint8_t channel, uint8_t gain, size_t length) {
	...
	for (size_t i = 0; i < length; i++) {
		...
		valCh = padcZmod->signedChannelData(channel, acqBuffer[i]);
...
void formatADCDataOverUART(ZMODADC1410 *padcZmod, uint32_t *acqBuffer, uint8_t channel, uint8_t gain, size_t length, size_t bufferCount)// size_t bufferCount
{
    ...
    int N = 10; // Change this to control how many samples are skipped
...
void ADC(uint8_t channel, uint8_t gain) {
    ZMODADC1410 adcZmod(ZMOD_ADC_BASE_ADDR, DMA_ADC_BASE_ADDR, IIC_BASE_ADDR, FLASH_ADDR_ADC,
        ZMOD_ADC_IRQ, DMA_ADC_IRQ);

    size_t length = 16384; // chunk size - adjust as needed
    uint32_t adcThreshold = 0xE0;
...
//    adcZmod.acquireImmediatePolling(acqBuffer1, length);
    ...
    size_t bufferCount = 0;
    adcZmod.acquireTriggeredPolling(acqBuffer1, channel, adcThreshold, 0, 0, length);
...
//        	adcZmod.acquireTriggeredPolling(acqBuffer2, channel, adcThreshold, 0, 0, 16384); //investigate optimal adcThreshold
            bufferCount++;

//    		writeADCData(&adcZmod, "E:/buffer_data.csv", acqBuffer1, channel, gain, length);
...
//    		writeADCData(&adcZmod, "E:/buffer_data.csv", acqBuffer2, channel, gain, length);
